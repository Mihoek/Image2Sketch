...
def mount_drive():
    """
    Monte Google Drive pour accéder aux données
    """
    try:
...
print("="*50)
print("DÉMARRAGE DU PROJET DE TRANSFORMATION D'IMAGE EN CROQUIS")
print("="*50 + "\n")
...
def create_directories():
    ...
    logger.info("Tous les répertoires requis ont été créés avec succès.")

# Étape 1: Configuration initiale
print("[1/8] 🛠️ Configuration initiale...")
create_directories()
print("✅ Répertoires créés avec succès\n")

def copy_data_from_drive():
    """
    Copie les données depuis Google Drive vers le répertoire de travail
    """
        ...
        for split in ["train", "test", "val"]:
            ...
            if os.path.exists(src_dir):
                ...
                if files:
                    ...
                    logger.info(f"{len(files)} fichiers copiés avec succès pour le split {split}.")
                else:
                    logger.warning(f"Aucun fichier trouvé dans {src_dir}.")
            else:
...
print("[2/8] 📁 Connexion à Google Drive...")
drive_mounted = mount_drive()
if not drive_mounted:
   print("❌ Échec du montage de Google Drive")
   exit(1)
print("✅ Google Drive monté avec succès\n")

# Étape 3: Copie des données
print("[3/8] 🔄 Copie des données depuis Google Drive...")
if not copy_data_from_drive():
  print("⚠️ Aucune donnée copiée ou erreur lors de la copie")
else:
  print("✅ Données copiées avec succès\n")

def process_combined_images():
    """
    Traite toutes les images combinées pour tous les splits (train, test, val)
    ...
    for split in ["train", "test", "val"]:
        ...
        if not image_paths:
            logger.warning(f"Aucune image trouvée dans {input_split_dir}")
            continue

        logger.info(f"{len(image_paths)} images trouvées dans {split}/")
        ...
        for i in tqdm(range(0, len(image_paths), batch_size), desc=f"Traitement {split}"):
            ...
            for path in batch_paths:
                try:
                    # Lecture de l'image avec vérification de mémoire
                    img = cv2.imread(path)
                    if img is None:
                    ...
                    # Enregistrement immédiat pour libérer la mémoire
        ...
        logger.info(f"Split {split} traité avec succès.")

# Classe pour le générateur de données
class DataGeneratorHelper:
    """
    Classe d'assistance pour interfacer les générateurs avec l'API Keras
    """
    ...
    def get_shape(self):
        """Renvoie la forme des données (X_shape, Y_shape)"""
        return self.shape
    ...
    def __iter__(self):
        """Retourne un itérateur sur les données"""
        ...
        for start_idx in range(0, num_samples, self.batch_size):
            ...
            for i in range(start_idx, end_idx):
                try:
                    ...
                    logger.error(f"Erreur lors du prétraitement de {self.real_paths[i]}: {e}")

            if X_batch and Y_batch:
                yield np.array(X_batch), np.array(Y_batch)

# Étape 4: Traitement des images
    print("[4/8] 🖼️ Traitement des images combinées...")
    process_combined_images()
    print("✅ Images traitées avec succès\n")

def preprocess_data(img_size=(256, 256)):
    """
    Prétraite les images pour l'entraînement du modèle

    Args:
        img_size: Taille cible pour les images

    Returns:
        Un tuple (train_data, val_data, test_data) contenant les générateurs de données
    """
    ...
    logger.info(f"Données d'entraînement: {train_data.get_length()} images")
    logger.info(f"Données de validation: {val_data.get_length()} images")
    logger.info(f"Données de test: {test_data.get_length()} images")

    return train_data, val_data, test_data

def build_unet_model(input_size=(256, 256, 3)):
    """
    Construit un modèle U-Net pour la transformation d'image en croquis

    Args:
        input_size: Forme des images d'entrée (hauteur, largeur, canaux)

    Returns:
        model: Modèle U-Net compilé
    """
...
print("[5/8] 🧠 Préparation des données pour l'entraînement...")
...
if train_data.get_length() == 0:
   ...
   print(f"✅ Données préparées: {train_data.get_length()} échantillons d'entraînement\n")

# Mise à jour de la fonction d'entraînement pour utiliser les générateurs
def train_model_with_generators(train_data, val_data, epochs=20):
    """
    Entraîne le modèle U-Net avec des générateurs de données pour économiser la mémoire

    Args:
        train_data: Générateur pour les données d'entraînement
        val_data: Générateur pour les données de validation
    ...
    # Créer des listes pour stocker l'historique manuellement
    history_dict = {'loss': [], 'accuracy': [], 'val_loss': [], 'val_accuracy': []}

    # Entraînement manuel pour un meilleur contrôle de la mémoire
    for epoch in range(epochs):
        ...
        # Validation si les données sont disponibles
        val_loss = []
        val_acc = []
    ...
    # Créer un objet history similaire à celui de Keras
...
print("[6/8] 🏋️ Entraînement du modèle U-Net...")
...
def save_model_to_drive(model, drive_path):
    """
    Sauvegarde un modèle Keras sur Google Drive.

    Args:
        model: Le modèle Keras à sauvegarder.
        drive_path: Le chemin complet sur Google Drive où sauvegarder le modèle
                    (par exemple, '/content/drive/MyDrive/Colab Notebooks/my_model.h5').
    """
    try:
        # Assurez-vous que le répertoire existe sur Drive
        drive_dir = os.path.dirname(drive_path)
        if not os.path.exists(drive_dir):
            os.makedirs(drive_dir, exist_ok=True)
            logger.info(f"Répertoire créé sur Drive: {drive_dir}")

        # Sauvegarder le modèle
        model.save(drive_path)
        logger.info(f"Modèle sauvegardé sur Google Drive à: {drive_path}")
    except Exception as e:
        logger.error(f"Erreur lors de la sauvegarde du modèle sur Drive: {e}")

# Étape 7: Sauvegarde du modèle sur Google Drive
print("[7/8] 💾 Sauvegarde du modèle sur Google Drive...")
# Définissez le chemin de sauvegarde sur votre Google Drive
MODEL_SAVE_PATH_DRIVE = os.path.join(DRIVE_BASE_DIR, 'image_to_sketch_model_saved.h5')

# Vérifiez si le modèle a été entraîné avec succès
if model is not None:
    save_model_to_drive(model, MODEL_SAVE_PATH_DRIVE)
    print(f"✅ Modèle sauvegardé sur Drive à: {MODEL_SAVE_PATH_DRIVE}\n")
else:
    print("❌ Impossible de sauvegarder le modèle car il n'a pas été entraîné\n")

# Étape 8: Évaluation du modèle (Optionnel)
# Vous pouvez ajouter ici le code pour évaluer le modèle sur l'ensemble de test
# using model.evaluate(test_data)
print("[8/8] 🎉 Fin du script.")
print("="*50 + "\n")
...
def evaluate_model(model, test_data):
    ...
    Args:
        model: Modèle entraîné
        test_data: Générateur pour les données de test

    Returns:
        evaluation: Métriques d'évaluation (loss, accuracy)
    """
    if test_data.get_length() == 0:
        logger.warning("Aucune donnée de test disponible")
        return None, None
...
def visualize_results(model, test_data, num_samples=5):
    ...
    """
    if test_data.get_length() == 0:
        logger.warning("Aucune donnée de test disponible pour la visualisation")
        return

    # Récupérer quelques échantillons pour la visualisation
    try:
        ...
        logger.warning("Pas assez d'échantillons disponibles pour la visualisation")
    except Exception as e:
...
def plot_training_history(history):
    ...
    plt.show()
    logger.info("Historique d'entraînement tracé avec succès.")

# Étape 7: Évaluation du modèle
print("[7/8] 📊 Évaluation du modèle...")
if test_data.get_length() > 0:
    ...
    print("⚠️ Aucune donnée de test disponible pour l'évaluation\n")